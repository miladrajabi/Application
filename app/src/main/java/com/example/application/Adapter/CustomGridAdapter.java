...
import com.example.application.Episods.CustomGridViewActivity;
...
public class CustomGridAdapter extends ArrayAdapter<CustomListViewObject> {
    List<CustomListViewObject> objects;

    public CustomGridAdapter(@NonNull Context context, int resource, @NonNull List<CustomListViewObject> objects) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (!CustomGridViewActivity.nameShow) {
            txtCustomTitle.setVisibility(View.GONE);
        }
        if (!CustomGridViewActivity.imgShow) {
            imgCustom.setVisibility(View.GONE);
        }
        if (!CustomGridViewActivity.descShow) {
            txtCustomDesc.setVisibility(View.GONE);
        }
        return view;
    }
}
